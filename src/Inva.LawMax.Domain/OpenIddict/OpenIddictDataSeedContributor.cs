...
namespace Inva.LawMax.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("LawMax") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "LawMax", DisplayName = "LawMax API", Resources = { "LawMax" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["LawMax_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["LawMax_App:RootUrl"]?.TrimEnd('/');
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["LawMax_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["LawMax_Swagger:RootUrl"]?.TrimEnd('/');
